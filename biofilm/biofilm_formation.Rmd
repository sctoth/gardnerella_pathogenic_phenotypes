---
title: "Biofilm Formation of Gardnerella Strains"
author: "Hanna Berman"
...
ODPlots <- plot_grid(protocolBar0, speciesBar0, percentODPlot, ncol = 1, rel_heights = c(0.06, 0.06, 1), align = "v", axis = "lr")
ODPlots
```
...
strain_perc_OD_DF <- percentODDF %>%
  ...
  select(Sample, Strain, Species, mPercentOD, sdPercentOD)

#save formatted percent OD data
# write_csv(strain_perc_OD_DF, file.path(formattedPhenDataOut, "percent_biofilm_od.csv"))

#save formatted biofilm and planktonic OD data
# percentODDF %>%
#   spread(measure, value) %>%
#   select(-`Percent OD`) %>%
#   dplyr::rename(biofilmOD=`Biofilm OD`,
#                 planktonicOD=`Planktonic OD`) %>%
#   with_groups(c(Sample, Species, Strain, Batch, BioRep), summarize, mBiofilmOD=mean(biofilmOD), sdBiofilmOD=sd(biofilmOD), mPlanktonicOD=mean(planktonicOD), sdPlanktonicOD=sd(planktonicOD)) %>%
#   select(Sample, Strain, Species, mBiofilmOD, sdBiofilmOD, mPlanktonicOD, sdPlanktonicOD) %>%
#   write_csv(file.path(formattedPhenDataOut, "biofilm_and_planktonic_od.csv"))
...
# addWorksheet(suppl_xlsx, "TableS5_percent_od_posthoc")
# 
# ODPostHoc %>%
#  arrange(p.adj) %>%
#  writeData(suppl_xlsx, x=.,  sheet="TableS5_percent_od_posthoc")
# 
...
safPlots <- plot_grid(protocolBar0, speciesBar0, safPlot2, ncol = 1, rel_heights = c(0.06, 0.06, 1), align = "v", axis = "lr")
...
strain_saf_DF <- safDF %>%
  with_groups(c(Sample, Species, Strain, Batch, BioRep), summarize, mSafOD=mean(safraninOD), sdSafOD=sd(safraninOD)) %>%
  select(Sample, Strain, Species, mSafOD, sdSafOD)
...
#save posthoc test results in supplementary tables file
# suppl_xlsx <- loadWorkbook(suppTableOut)
# addWorksheet(suppl_xlsx, "TableS6_safranin_posthoc")
# 
# safPostHoc %>%
#  arrange(p.adj) %>%
#  writeData(suppl_xlsx, x=.,  sheet="TableS6_safranin_posthoc")
# 
# saveWorkbook(suppl_xlsx, suppTableOut, overwrite = TRUE)
```
  Plot pairwise post-hoc results
...
plot_grid(protocolBar0, speciesBar0, percentODPlot + speciesColor + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()), safPlot2, ncol = 1, rel_heights = c(0.07, 0.07, 1, 0.5), align = "v", axis = "lr", labels = c("A", "", "", "B"), label_size = 15)
...
#plot_grid(percODPostHocPlot, ggplot()+geom_blank()+theme_void(), safraninPostHocPlot, nrow = 3, rel_heights = c(1, 0.3, 1), labels = c("A", " ","B"), label_size = 20)
