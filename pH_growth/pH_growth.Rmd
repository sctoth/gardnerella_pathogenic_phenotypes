---
title: "Growth of Gardnerella Strains at Varying pH"
author: "Hanna Berman"
...
pH_DF_save <- DF %>%
  ...
  mutate(pH=as.numeric(pH),
         Sample=paste(Strain, Batch, BioRep, pH, sep=".")) %>%
  select(Sample, Strain, Species, pH, mOD, sdOD)
...
# addWorksheet(suppl_xlsx, "TablesS7_pH_posthocs")
# 
# odPostHocs %>%
#  arrange(pH, p.adj) %>%
#  writeData(suppl_xlsx, x=.,  sheet="TablesS7_pH_posthocs")
# 
...
#save posthoc test results in supplementary tables file
# suppl_xlsx <- loadWorkbook(suppTableOut)
# addWorksheet(suppl_xlsx, "TablesS8_pH_ratio_posthocs")
# 
# ratioPostHocs %>%
#  arrange(pH, p.adj) %>%
#  writeData(suppl_xlsx, x=.,  sheet="TablesS8_pH_ratio_posthocs")
# 
# saveWorkbook(suppl_xlsx, suppTableOut, overwrite = TRUE)
```
  Plot pairwise post-hoc results
...
plot_grid(pHGrowthPlot+theme(axis.title.y = element_text(size=18), strip.text.x = element_text(size=6.4)), pHratioPlot+theme(axis.title.y = element_text(size=18), strip.text.x = element_text(size=6.4)), labels = c("A", "B"), label_size = 15)
#ggsave(filename = file.path(figureOut, paste(Sys.Date(), "Figure5_pHGrowth.png", sep = "_")))
```

# Figure S4: Post Hoc Test Results
```{r, message=FALSE, warning=FALSE, fig.width=16, fig.height=32.5} 
